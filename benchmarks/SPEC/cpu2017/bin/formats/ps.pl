...
#  Copyright 1999-2020 Standard Performance Evaluation Corporation
#
#  Authors:  Christopher Chan-Nui
#            Cloyce D. Spradling
#
# $Id: ps.pl 6442 2020-08-06 17:20:40Z CloyceS $
...
my $ps_version = '$LastChangedRevision: 6442 $ '; # Make emacs happier
    ...
    foreach my $line (@lines) {
        ...
        if ($line =~ m#(?:(?:https?|ftp)://|mailto:)#i) {
            ...
            foreach my $chunk (split(m#((?:(?:https?|ftp)://|mailto:)[^[:space:]<>()]+)#i, $line)) {
                ...
                # to the previous text.  munge_links will handle it.
                if ($#{$lineref} >= 0) {
                ...
                if ($chunk =~ m#^(?:(?:https?|ftp)://|mailto:)#i) {
                    my $url = $chunk;
                    # Remove any stray punctuation from the end of the URL; it will still show up as linked, because
                    # pushing it to the next chunk would be a huge pain, but the actual URL will be correct.
                    # Not using [:punct:] or any synonyms here because they all include "/", which is occasionally very
                    # important at the end of a URL.  See note 5 in "POSIX Character Classes" at
                    # https://perldoc.perl.org/perlrecharclass.html for the expansion used here.
                    $url =~ s/([-!"#$%&'()*+,.:;<=>?@[\\\]^_`{|}~]+)$//;
                    $chunk =~ s/^mailto://i;
