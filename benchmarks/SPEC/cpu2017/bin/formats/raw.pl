...
#            Cloyce D. Spradling
#
# $Id: raw.pl 6423 2020-07-21 15:45:47Z CloyceS $
...
my $version = '$LastChangedRevision: 6423 $ '; # Make emacs happier
            ...
            for (my $i = 0; $i < @tmp; $i++) {
                for my $key (sort keys %{$tmp[$i]}) {
                    ...
                    if (ref($val) eq 'ARRAY') {
                        ...
                        push @hashlist, sprintf("%s.%s: %s", $prefix, $partkey, $val);
                        $hashfields{$partkey} = [ $val ];
                    }
                }
            }
