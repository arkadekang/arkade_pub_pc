...
#
# $Id: util_common.pl 6430 2020-08-04 17:16:14Z CloyceS $

my $version = '$LastChangedRevision: 6430 $ '; # Make emacs happier
    ...
    while ($count < 40 && $text =~ /(ATTACH\s+(\S+)\s+AS\s+(\[[^]]+\]|\S+))/g) {
        ...
        if (exists($::url_to_file{$url})) {
            ...
            if (@formats > 1) {
                ...
                # Runcpu doesn't.  Not hardcoding this would require making
