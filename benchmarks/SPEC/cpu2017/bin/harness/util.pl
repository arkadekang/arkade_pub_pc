...
#
# $Id: util.pl 6534 2020-09-23 13:41:05Z CloyceS $
...
my $version = '$LastChangedRevision: 6534 $ '; # Make emacs happier
...
# Read ranges from all the channels in the meter and return them
sub meter_read_ranges {
    my ($meter) = @_;
    ...
    for(my $chan = 0; $chan < $meter->{'channels'}; $chan++) {
        if ($chan > 0 || $meter->{'channels'} == 1) {
            ...
            if (defined($currange) && ref($currange) eq 'HASH') {
...
sub meter_start {
    ...
    foreach my $meter (@meterlist) {
        ...
        if ($meter->{'can_rangeset'} and $meter->{'ptd_mode'} ne 'temperature') {
            # Set the ranges as specified
            if (!meter_set_range($meter, $ranges)) {
                return 0;
            }

            # Get the ranges so we can check for voltage auto-ranging
            my ($isok, @ranges) = meter_read_ranges($meter);
            foreach my $range (grep { defined && ref($_) eq 'HASH' } @ranges) {
                if ($range->{'v_auto'} > 0) {
                    ::Log(0, "ERROR: PTD($meter->{'name'}) is in voltage auto-ranging mode.  Please set the power"
                        ." analyzer to a fixed voltage by adding a voltage_range setting to the global section"
                        ." of the config file.\n");
                    $isok = 0;
                    last;
                }
            }
            ::do_exit(1) unless $isok;
        }
    }
