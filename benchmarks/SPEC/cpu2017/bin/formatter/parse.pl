...
#
# $Id: parse.pl 6407 2020-01-07 17:40:38Z CloyceS $
...
my $version = '$LastChangedRevision: 6407 $ '; # Make emacs happier
...
sub parse_raw {
    ...
    if ($config->info_wrap_columns) {   # Most likely!
        ...
        foreach my $system ('', keys %{$r->{'mpi_items'}}) {
            my $itemre;
            if ($system eq '') {
                $itemre = qr/^(?:hw_|sw_|fw_|power_)/;
            } else {
                $itemre = qr/^\Q$system\E(?:hw_|sw_|fw_|power_|interconnect|label)/;
            }
